...
Textarea.prototype._updateCursor = function(get) {
  ...
  let cursorPos = this.__cursorIndex;
  for (let i = 0; i < this._clines.length; i++) {
    const length = helpers.stripTags(this._clines[i]).length;
    if (length == cursorPos) break;
    cursorPos -= length;
  }

  cy = lpos.yi + this.itop + line;
  cx = lpos.xi + this.ileft + cursorPos;
